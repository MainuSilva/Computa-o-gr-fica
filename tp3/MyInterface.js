...
export class MyInterface extends CGFinterface {
  ...
  init(application) {
    ...
    this.gui
      .add(this.scene, "selectedObject", this.scene.objectIDs)
      .name("Selected Object")
      .onChange(this.scene.updateObjectComplexity.bind(this.scene));

    this.gui.add(this.scene, "scaleFactor", 0.1, 10.0).name("Scale");
    this.gui
      .add(this.scene, "objectComplexity", 0.01, 1.0)
      .onChange(this.scene.updateObjectComplexity.bind(this.scene));

    this.gui.add(this.scene, "brightness", 0, 1).name("Brighness");
    ...
    f2.addColor(this.scene.customMaterialValues, "Ambient").onChange(
      this.scene.updateCustomMaterial.bind(this.scene)
    );
    f2.addColor(this.scene.customMaterialValues, "Diffuse").onChange(
      this.scene.updateCustomMaterial.bind(this.scene)
    );
    f2.addColor(this.scene.customMaterialValues, "Specular").onChange(
      this.scene.updateCustomMaterial.bind(this.scene)
    );
    f2.add(this.scene.customMaterialValues, "Shininess", 0, 100).onChange(
      this.scene.updateCustomMaterial.bind(this.scene)
    );
