...
export class ShaderScene extends CGFscene {
  ...
  init(application) {
    ...
    this.objects = [new Teapot(this), new MyPlane(this, 50)];
  ...
  onSelectedShaderChanged(v) {
    // update shader code
    this.vShaderDiv.innerHTML =
      "<xmp>" + getStringFromUrl(this.testShaders[v].vertexURL) + "</xmp>";
    this.fShaderDiv.innerHTML =
      "<xmp>" + getStringFromUrl(this.testShaders[v].fragmentURL) + "</xmp>";
  ...
  update(t) {
    ...
    if (this.selectedExampleShader  in [6, 10])
